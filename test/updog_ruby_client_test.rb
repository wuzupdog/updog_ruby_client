...
class UpdogRubyClientTest < Minitest::Test
  ...
  def test_notify_sends_notice_with_context_and_breadcrumbs
    ...
    assert_equal :ok, UpdogRubyClient.flush(1)
    assert_equal 1, fake.calls.length
    call = fake.calls.first
    assert_equal "https://example.test/api/v1/notices/bulk", call[:url]
    assert_equal "test-key", call[:headers]["X-API-Key"]
    refute_nil call[:headers]["X-Updog-Request-ID"]
    payload = call[:payload][:notices].first
    assert_equal "RuntimeError", payload[:error_class]
    assert_equal "kaboom", payload[:message]
    assert_equal({ user_id: 42 }, payload[:context])
    assert_equal "abc123", payload[:fingerprint]
    assert_equal 1, payload[:breadcrumbs].length
    assert_equal "info", payload[:breadcrumbs][0][:level]
    refute_nil payload[:event_id]
  ...
  def test_notify_is_fail_safe
    ...
    assert_equal :ok, UpdogRubyClient.flush(1)
    assert_equal 1, UpdogRubyClient.delivery_stats[:dropped][:retries_exhausted]
  end
  ...
  def test_bounded_queue_drops_new_records_without_blocking
    fake = FakeTransport.new
    UpdogRubyClient.configure do |c|
      c.transport = fake
      c.max_queue_records = 1
    end

    assert_equal :ok, UpdogRubyClient.notify(StandardError.new("one"))
    assert_equal :ok, UpdogRubyClient.notify(StandardError.new("two"))

    stats = UpdogRubyClient.delivery_stats
    assert_equal 1, stats[:queue_records]
    assert_equal 1, stats[:dropped][:queue_full]
    assert_equal :ok, UpdogRubyClient.flush(1)
  end
end
